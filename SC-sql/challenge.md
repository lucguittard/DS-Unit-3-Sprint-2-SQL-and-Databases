...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables? 
    The employee and territory tables may be said to have a one-to-many relationship. Upon inspection of the two tables, one finds that there are a great number more entries in the latter relative to the former. In this case, one may theorize that each sales-rep is likely managing several territories or regions. And one may test this hypothesis by attempting to join the two tables (eg. Employee JOIN Territory ON...).
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
    MongoDB and is great when working with unstructured data (eg. a store of news articles or blog posts) and when you want to do some quick prototyping without having to worry about defining a schema first thing. They have their quirks though. You wouldn't want to keep it as a long-term solution as relational databases have better ACID guarantees. 
- What is "NewSQL", and what is it trying to achieve? 
    NewSQL is trying to bridge the gap in the exclusive benefits afforded when working with either relational or non-relational databases. Its promise is the (horizontal) scalability of non-relational databases coupled with the ACID guarantees of relational databases. Citus and Vitess are two examples of extensions designed to build on existing SQL frameworks (PostgreSQL and MySQL, respectively), to grant additional features (such as automatic sharding) that are typically associated with non-relational databases.
